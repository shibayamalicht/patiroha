...
def _tfidf_mean_labels(
    ...
    try:
        ...
        fallback = {int(lid): f"Cluster {lid}" for lid in unique_labels if lid != -1}
        if -1 in unique_labels:
            fallback[-1] = noise_label
        return fallback
...
def _c_tfidf_labels(
    ...
    try:
        ...
        # Preserve the noise label already placed in labels_map.
        labels_map.update({lid: f"Cluster {lid}" for lid in cluster_ids})
        return labels_map
